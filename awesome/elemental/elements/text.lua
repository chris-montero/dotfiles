...
local function _draw(self, cr, avail_w, avail_h)
    ...
    -- if self.id ~= nil then
    --     cr:set_source(esource.to_cairo_source(tcolor.rgba(0, 1, 0, 0.3)))
    --     cr:rectangle(0, 0, log.width, log.height)
    --     cr:fill()
    -- end

    -- if not etypes.is_size_shrink(self.width) then
    --     self._text_layout:set_width(lgi.Pango.units_from_double(avail_w))
    -- end
    -- if not etypes.is_size_shrink(self.height) then
    ...
    if self.bg ~= nil then
        ...
        if layout_dimensions.width ~= 0 and layout_dimensions.height ~= 0 then
            ...
            local line_y = 0
            cr:save()
            cr:set_source(esource.to_cairo_source(self.bg))
    ...
    cr:set_source(esource.to_cairo_source(self.fg))
    -- cr:update_layout(self._text_layout)
    cr:show_layout(self._text_layout)
end

local function _calculate_minimum_dimensions(self, constraint_w, constraint_h)
    ...
    if type(self.width) == "number" then
        w = self.width
    elseif etypes.is_size_fill(self.width) then
        w = constraint_w
    end

    if type(self.height) == "number" then
        h = self.height
    elseif etypes.is_size_fill(self.height) then
        h = constraint_h
    end
...
local function new(args)
    ...
    local defaults = {
        ...
        bg = nil,
        fg = tcolor.rgb_from_string("#000000"),
        letter_spacing = 0,
        line_spacing = 0, -- TODO
        -- text_halign -- TODO
        text = "",
    ...
    local pango_letter_spacing = lgi.Pango.attr_letter_spacing_new(
        lgi.Pango.units_from_double(ext.letter_spacing)
    )
    ...
    ext._text_layout = text_layout
    ext._pango_attrs = pango_attrs
    ext._font_description = desc
...
local function set_text(text_el, txt)
    ...
    -- if etypes.is_size_shrink(text_el.width) then
