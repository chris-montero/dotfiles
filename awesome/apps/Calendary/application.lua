...
local function new(args)
    ...
    if global_model.theme_name == theme_names[1] then -- LateForLunch
        ...
        local layout = require("apps.Calendary." .. global_model.theme_name .. ".layout")
    ...
    tstation.subscribe_signals(global_station, {
        EventCalendaryShown = function()
            keygrabber.run(function(mods, key, evt)
                ...
                if k.modifiers.Mod1 and k.key == 'c' then -- alt + c
                    keygrabber.stop()
                    tstation.emit_signal(global_station, "RequestCalendaryHide")
                end
