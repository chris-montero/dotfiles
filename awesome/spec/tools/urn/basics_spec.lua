...
describe("tools.urn", function()
    it(
        ...
        function()
            ...
            local function make_num(parsed_data, parsed_zero)
                if parsed_data.parse_type == turn.PARSE_FAILURE then
            ...
            local zero_parser = turn.char(make_num, '0')
            local parsed_data, parsed_value = turn.parse("0", zero_parser)

            assert(parsed_data.parse_type == turn.PARSE_SUCCESS)
    ...
    it(
        ...
        function()
            ...
            local function propagate(parsed_data, value)
                ...
                if parsed_data.parse_type == turn.PARSE_FAILURE_EOF then
                    return problem_eof(parsed_data.position)
                elseif parsed_data.parse_type == turn.PARSE_FAILURE then
            ...
            local one_parser = turn.char(propagate, '1')
            local chomp_ones = turn.sequence(make_num, {
                one_parser,
                one_parser,
                one_parser,
            })

            local success_parse_data, success_val = turn.parse("111", chomp_ones)
            ...
            local failure_eof_parse_data, failure_eof_val = turn.parse("11", chomp_ones)
            assert.is_same(failure_eof_parse_data, {
                offset = 3,
                parse_type = turn.PARSE_FAILURE_EOF,
                position = {
                    row = 1,
                    column = 3,
                }
            ...
            local failure_char_parse_data, failure_char_val = turn.parse("121", chomp_ones)
            assert.is_same(failure_char_parse_data, {
                char = '2',
                offset = 2,
                parse_type = turn.PARSE_FAILURE,
                position = {
                    row = 1,
                    column = 2
                }
    ...
    it(
        ...
        function()
            ...
            local one_parser = turn.char(propagate_char, '1')
            local chomp_ones = turn.one_or_more(make_num, one_parser)

            local success_parsed_data, success_val = turn.parse("1111", chomp_ones)
            ...
            local failure_unexpected_char_parsed_data,
                failure_unexpected_char_value = turn.parse("21", chomp_ones)
    ...
    it(
        ...
        function()
            ...
            local process_digits = function(parse_data, val)
                if parse_data.parse_type == turn.PARSE_FAILURE then
            ...
            local digit_parser = turn.one_of(prop_char, {
                turn.char(prop_char, '0'),
                turn.char(prop_char, '1'),
                turn.char(prop_char, '2'),
                turn.char(prop_char, '3'),
                turn.char(prop_char, '4'),
                turn.char(prop_char, '5'),
                turn.char(prop_char, '6'),
                turn.char(prop_char, '7'),
                turn.char(prop_char, '8'),
                turn.char(prop_char, '9'),
            })

            local parse_many_digits = turn.one_or_more(process_digits, digit_parser)

            local success_parse_data, success_val = turn.parse("3456", parse_many_digits)
            assert.is_same(success_parse_data, {
                parse_type = turn.PARSE_SUCCESS,
                position_start = {
                    row = 1,
            ...
            local failure_no_digits_parse_data,
                failure_val = turn.parse("no digits", parse_many_digits)
    ...
    it(
        ...
        function()
            ...
            local a_parser = turn.char(id, 'a')

            local function letters_to_word(_, letters)
                return table.concat(letters)
            end

            local many_a_parser = turn.zero_or_more(letters_to_word, a_parser)

            local parsed_data, parsed_value = turn.parse("aaa", many_a_parser)
    ...
    it(
        ...
        function()
            ...
            local digit_parser = turn.one_of(id, {
                turn.char(id, '0'),
                turn.char(id, '1'),
                turn.char(id, '2'),
                turn.char(id, '3'),
            ...
            local make_tuple = function(parsed_data, parsed_nums)
                if parsed_data.parse_type == turn.PARSE_SUCCESS then
                    return make_success({parsed_nums[1].value, parsed_nums[2].value})
                elseif parsed_data.parse_type == turn.PARSE_FAILURE_EOF then
                    return make_problem_eof(parsed_data.position)
            ...
            local number_parser = turn.one_or_more(make_num, digit_parser)

            local tuple_parser = turn.sequence(make_tuple, {
                turn.char(di, '('),
                turn.zero_or_more(di, turn.char(di, ' ')),
                number_parser,
                turn.zero_or_more(di, turn.char(di, ' ')),
                turn.char(di, ','),
                turn.zero_or_more(di, turn.char(di, ' ')),
            ...
            -- success
            local success_parse_data, success_value = turn.parse("(12, 34    )", tuple_parser)
            assert.is_same(success_parse_data, {
                parse_type = turn.PARSE_SUCCESS,
                offset = 13,
                position_start = {
            ...
            local failure_number_parse_data, failure_number = turn.parse("(12, a)", tuple_parser)
            ...
            local failure_tuple_parse_data, failure_tuple = turn.parse("(12 34)", tuple_parser)
            ...
            -- failure end of file
            local failure_eof_parse_data, failure_eof = turn.parse("(12", tuple_parser)
            assert.is_same(failure_eof_parse_data, {
                parse_type = turn.PARSE_FAILURE_EOF,
                offset = 4,
                position = {
    ...
    it(
        ...
        function()
            ...
            local whitespace = turn.char(di, ' ')

            local function get_el_from_seq(parsed_data, a)
                return a[1]
            end

            local spaced_or_not = turn.sequence(get_el_from_seq, {
                turn.maybe(di, whitespace),
                turn.char(id, 'a'),
                turn.maybe(di, whitespace)
            })
            ...
            local function make_test(result)
                return function(input)
                    local _, value = turn.parse(input, spaced_or_not)
                    assert(result == value)
                end
            end
    ...
    it(
        ...
        function()
            ...
            local number_parser = turn.one_or_more(make_num, digit_parser)
            local element_parser = turn.sequence(make_element, {
                number_parser,
            })

            local many_elements_parser = turn.zero_or_more(propagate_elements,
                element_parser,
                turn.sequence(di, {
                    turn.zero_or_more(di, whitespace),
                    turn.char(di, ','),
                    turn.zero_or_more(di, whitespace),
                })
            )

            local list_parser = turn.sequence(make_list, {
                turn.char(di, '['),
                turn.zero_or_more(di, whitespace),
            ...
            local function make_test(result)
                return function (input)
                    local parsed_data, parsed_result = turn.parse(input, list_parser)
            ...
            local fail_eof_parse_data, fail_eof_result = turn.parse("[ 12", list_parser)
            ...
            local fail_number_parse_data, fail_number = turn.parse("[ a ]", list_parser)
    ...
    it(
        ...
        function()
            ...
            local whitespace = turn.one_of(di, {
                turn.char(di, ' '),
                turn.char(di, '\n')
            })
            ...
            list_parser = function()
                return turn.sequence(make_list, {
                    turn.zero_or_more(di, whitespace),
                    turn.char(di,'['),
                    turn.zero_or_more(di, whitespace),
                    turn.zero_or_more(id, list_parser),
                    turn.zero_or_more(di, whitespace),
                    turn.char(di, ']'),
                    turn.zero_or_more(di, whitespace),
                })()
            ...
            local function make_test(result)
                return function(input)
                    local _, res = turn.parse(input, list_parser)
    ...
    it(
        ...
        function()
            ...
            local many_values_parser = turn.zero_or_more(propagate_elements,
                value_parser,
                turn.sequence(di, {
                    turn.zero_or_more(di, whitespace),
                    turn.char(di, ','),
            ...
            local function make_test(result)
                return function (input)
                    local _, parsed_result = turn.parse(input, list_parser)
                    assert.is_same(parsed_result, result)
                end
            end
